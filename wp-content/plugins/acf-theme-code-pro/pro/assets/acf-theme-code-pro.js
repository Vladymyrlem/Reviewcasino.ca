...
(function($) {
  $(document).ready(function() {
    ...
    $("#acf_block_name").bind("change paste keyup", function() {
      ...
      var block_name_raw = $(this)
        .val()
        .replace(/[^a-zA-Z0-9 _-]/g, "");
      var block_name_lower_dashes = block_name_raw
        .toLowerCase()
        .replace(/[ _]/g, "-");
      var block_name_lower_underscores = block_name_raw
        .toLowerCase()
        .replace(/[ -]/g, "_");
      var block_name_lower_keywords = block_name_raw
        .toLowerCase()
        .replace(/([_ -])+/g, "', '");
      ...
      if (block_name_raw != "") {
        ...
        $(".acf-tc-block-name--lower-underscores").text(
          block_name_lower_underscores
        );
    ...
    $("#acf_option_name").bind("change paste keyup", function() {
      // set some vars to the value of the input
      var option_name_raw = $(this)
        .val()
        .replace(/[^a-zA-Z0-9 ]/g, "");
      var option_name_lower_dashes = option_name_raw
        .toLowerCase()
        .split(" ")
        .join("-");
