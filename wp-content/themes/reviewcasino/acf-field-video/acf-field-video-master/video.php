...
class acf_field_video extends acf_field {
	...
	*/
	function __construct() {
		...
		$this->defaults = array(
			...
			'preview_type' => 'embed',
			//whether to display a preview for the video. Display video embed, thumbnail image or none
			'return_value' => 'url',
			//return the url, embed code, thumbnail url or array or all
		);
	...
	*/
	function create_field( $field ) {
		...
		<?php if ( ! empty( $thumbnail ) ) { ?>
            <a href="<?php echo $field['value']; ?>" title="" target="_blank"
               style="display:block;padding-top:4px;margin-bottom:10px;"><img src='<?php echo $thumbnail; ?>' alt=''
        ...
        <input type="text" id="<?php echo $field['key']; ?>" class="<?php echo $field['class']; ?>"
               name="<?php echo $field['name']; ?>" value="<?php echo $field['value']; ?>"/>
		<?php
	}
	...
	*/
	function input_admin_enqueue_scripts() {
		...
		wp_register_script( 'acf-input-video', $this->settings['dir'] . 'js/input.js', array(
			'acf-input',
			'jquery-fitvids'
